            ...
            for i in range(len(fleLst)):
                ...
                if line == name:
                    ...
                    if initName != 'Arial.ttf':
                        log("MyFont: isFontInstalled, replace " + initFont + ", with = " + filename)
                        replaceAll(fontxml_path,initFont,filename)
                        nameCheck = True
        else:
            nameCheck = True
    log("MyFont: isFontInstalled = " + str(nameCheck))
    return nameCheck

def copyFont(FontPath, SkinPath):
    log("MyFont: copyFont, FontPath = " + FontPath + ", SkinPath = " + SkinPath)
    dest = os.path.join(SkinPath, 'fonts', FontName)
    if os.path.exists(dest):
        log("MyFont: copyFont dest exists") 
        return
