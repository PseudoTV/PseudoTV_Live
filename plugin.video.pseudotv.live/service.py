...
class Player(xbmc.Player):
    ...
    def playAction(self):
        if not isPseudoTV(): 
            return log('Player: playAction, returning not PseudoTV Live') #isPseudoTV check, todo added logic to exclude other playback sources
        ...
        if lastChannelId == currentChannelId: 
            log('Player: playAction, no channel change')
        else:   
            self.playingChannelItem = currentChannelItem
            setCurrentChannelItem(self.playingChannelItem)
        log('Player: playAction, lastChannelId = %s, currentChannelId = %s, callback = %s'%(lastChannelId,currentChannelId,currentChannelItem['callback']))
...
class Service:
    def __init__(self):
        ...
        self.myPlugin    = Plugin(sys.argv,self.myPlayer)
