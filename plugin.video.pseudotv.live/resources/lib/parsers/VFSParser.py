...
class VFSParser:
    def determineLength(self, filename: str, fileitem: dict={}, jsonRPC=None)-> int and float:
        ...
        duration = (fileitem.get('runtime') or fileitem.get('duration') or (fileitem.get('streamdetails',{}).get('video',[]) or [{}])[0].get('duration') or fileitem.get('resume',{}).get('total') or 0)
