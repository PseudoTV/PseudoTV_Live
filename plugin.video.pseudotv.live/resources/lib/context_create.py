...
def _add(sysARG, listitem: dict={}):
    ...
    if not path: return Globals.DIALOG.notificationDialog(LANGUAGE(32030))
    elif Globals.DIALOG.yesnoDialog('Would you like to add:\n[B]%s[/B]\nto the first available %s channel?'%(listitem.Globals._getLabel(),ADDON_NAME)):
        if not Globals.PROPERTIES.isRunning('Create.add'):
            with Globals.PROPERTIES.chkRunning('Create.add'):
                ...
                name, citem = manager.validateInputs('name',Globals._cleanLabel(listitem.Globals._getLabel()),citem)
                if not path is None:
