...
class Manager(xbmcgui.WindowXMLDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        def __loadChannels(name=Globals.settings.getSetting('Default_Channels')): #load local or remote channel configurations
            ...
            self.log('__loadChannels, oldChannels = %d, first 3 = %s' % (len(self.oldChannels), [(c.get('number'), c.get('name')) for c in self.oldChannels[:3]]))
            if name == self.friendly or not kwargs.get('start', False): 
                ...
                def __buildItem(server):
                    ...
                    label2 = '%s - %s: Channels (%s)' % (
                        LANGUAGE(32211).format(color=color, text=status_text),
                        server.get('host'),
    ...
    def _findAvailChannel(self, start: int = -1) -> int:
        if start == -1: 
    ...
    def buildArray(self) -> list:
        ...
        return [{'number': idx + 1} for idx in range(CHANNEL_LIMIT)]
  
    def createChannelList(self, channelArray: list, channelList: list) -> list:
        """Fill blank array with citems from channels.json, skipping duplicates."""
        self.log('createChannelList, channelList = %d, channelArray = %d' % (len(channelList), len(channelArray)))
        for item in channelList[:10]:
            self.log('createChannelList, item: number=%s, name=%s, type=%s, id=%s' % (item.get('number'), item.get('name'), item.get('type'), bool(item.get('id'))))
        def __update(item: dict):
            number = item.get("number", 0)
            if number > 0 and number <= CHANNEL_LIMIT:
                existing = channelArray[number - 1]
                if not existing.get('id'):
                    channelArray[number - 1].update(item)
                elif existing.get('id') != item.get('id'):
                    self.log('createChannelList, duplicate number %d, keeping existing' % number)
    ...
    @staticmethod
    def _buildChannelListItem(citem: dict, server_name: str = '') -> xbmcgui.ListItem:
        """Build a ListItem for a channel in the channel list."""
        ...
        chnum         = citem.get("number", 0)
        LOG(f"Manager: _buildChannelListItem, number={chnum}, name={chname}, type={citem.get('type','')}, path={bool(citem.get('path'))}", xbmc.LOGDEBUG)
        ...
        return Globals.listitems.buildMenuListItem(
            '[COLOR=%s][B]%s|[/COLOR][/B]' % (channelColor, citem.get("number", 0)),
            '[COLOR=%s]%s[/COLOR]' % (labelColor, chname),
            citem.get("logo", LOGO_COLOR),
            '|'.join(citem.get("path", [])),
            props={
                'citem': citem,
                'chname': chname,
                'chnum': '%i' % (citem.get("number", 0)),
                'radio': citem.get('radio', False),
                'description': LANGUAGE(32169).format(name=citem.get("number", 0), id=server_name)
            }
        )


    def fillChanList(self, channelList: list, refresh: bool = False, focus: Optional[int] = None, channel: Optional[dict] = None):
        """Populate the channel list UI control with channel items."""
        self.log('fillChanList, focus = %s, channel = %s' % (focus, channel))
        self.togglechanList(reset=refresh)
        server_name = self.server.get('name', self.friendly)
        with self.toggleSpinner(condition=(Globals.properties.isRunning('Manager.toggleSpinner') == False)):
            lizLST = [self._buildChannelListItem(citem, server_name) for citem in channelList]
    ...
    @contextmanager
    def toggleSpinner(self, state: bool = True, lock: bool = True, condition: Optional[bool] = None):
    ...
    def togglechanList(self, state: bool = True, focus: int = 0, reset: bool = False):
    ...
    def setFocusPOS(self, listitems: list, chnum: Optional[int] = None, ignore: bool = True) -> int:
    ...
    def selItem(self, cntrl: xbmcgui.ControlList, focus: int = 0):
        try: 
    ...
    def getLogoColor(self, citem: dict) -> str:
    ...
    def buildChannelItem(self, citem: dict={}, focuskey: str='path'):
        ...
        def __buildItem(key: str) -> Optional[xbmcgui.ListItem]:
            key   = key.lower()
            value = citem.get(key, ' ')
            if key in ["number", "type", "logo", "id", "catchup"]: 
                ...
                elif key == "rules": value = '|'.join([rule.name for k, rule in self.rule.loadRules([citem]).get(citem.get('id',''), {}).items()])
    ...
    def itemInput(self, channelListItem: xbmcgui.ListItem = xbmcgui.ListItem()) -> dict:
        def __getName(citem: dict = {}, name: str = '') -> tuple:
            return Globals.dialog.inputDialog(message=LANGUAGE(32079), default=name), citem
       
        def __getPath(citem: dict = {}, paths: list = []) -> tuple:
            return self.getPaths(citem, paths)
        
        def __getRule(citem: dict = {}, rules: dict = {}) -> tuple:
            return self.getRules(citem, rules)
            
        def __getBool(citem: dict = {}, state: bool = False) -> tuple:
            return not bool(state), citem

        def __getGroups(citem: dict = {}, groups: list = []) -> tuple:
        ...
        KEY_INPUT = {
            ...
            "rules"    : {'func': __getRule,   'kwargs': {'citem': citem, 'rules': self.rule.loadRules([citem]).get(citem.get('id',''), {})}},
    ...
    def getPaths(self, citem: dict = {}, paths: list = []) -> tuple:
        def __buildItem(path: str) -> xbmcgui.ListItem:
            idx = pathLST.index(path) + 1
            return Globals.listitems.buildMenuListItem('', path, url='|'.join([path]), icon=Globals._getDummyIcon(str(idx)), props={'citem': citem, 'idx': idx})
        ...
        while not self.monitor.abortRequested() and select is not None:
            with self.toggleSpinner(condition=(Globals.properties.isRunning('Manager.toggleSpinner') == False)):
                ...
                lizLST.insert(0,Globals.listitems.buildMenuListItem(f'[COLOR=white][B]{LANGUAGE(34067)}[/B][/COLOR]', '', icon=Globals._getDummyIcon(str(len(pathLST) + 1)), props={'key': 'add', 'citem': citem, 'idx': 0}))
                if len(pathLST) > 0 and epaths != pathLST: 
                    lizLST.insert(0, Globals.listitems.buildMenuListItem(f'[COLOR=white][B]{LANGUAGE(32222)}[/B][/COLOR]', LANGUAGE(33114), icon=ICON, props={'key': 'save', 'citem': citem}))
    ...
    def getRules(self, citem: dict = {}, rules: dict = {}) -> tuple:
        def __buildItem(data: tuple) -> xbmcgui.ListItem:
        ...
        if citem.get('id') is None or len(citem.get('path', [])) == 0: 
            ...
            while not self.monitor.abortRequested() and select is not None:
                with self.toggleSpinner(condition=(Globals.properties.isRunning('Manager.toggleSpinner') == False)):
                    nrule  = None
                    crules = self.rule.loadRules([citem], append=True).get(citem.get('id',''), {}) 
                    ...
                    if len(ruleLST) > 0 and erules != ruleLST: 
                        lizLST.insert(1, Globals.listitems.buildMenuListItem(f'[COLOR=white][B]{LANGUAGE(32174)}[/B][/COLOR]', "", icon=ICON, props={'key': 'save', 'citem': citem}))
    ...
    def getRule(self, citem: dict = {}, rule: dict = {}) -> tuple:
        self.log('getRule, name = %s' % (rule.name))
        select = -1
        while not self.monitor.abortRequested() and select is not None:
            with self.toggleSpinner(condition=(Globals.properties.isRunning('Manager.toggleSpinner') == False)):
                lizLST = [Globals.listitems.buildMenuListItem('%s = %s' % (rule.optionLabels[idx], rule.optionValues[idx]), rule.optionDescriptions[idx], Globals._getDummyIcon(idx + 1), str(rule.myId), {'value': optionValue, 'idx': idx, 'myId': rule.myId, 'citem': citem}) for idx, optionValue in enumerate(rule.optionValues)]
    ...
    def setName(self, path, citem: dict={}) -> dict:
        with self.toggleSpinner(condition=(Globals.properties.isRunning('Manager.toggleSpinner') == False)):
            ...
            self.log('setName, id = %s, name = %s' % (citem.get('id',''), citem.get('name','')))
            return citem


    def setLogo(self, name: Optional[str] = None, citem: dict = {}, force: bool = False) -> dict:
    ...
    def validateInputs(self, key: str, value: Any, citem: dict) -> tuple:
        self.log('validateInputs, key = %s, value = %s' % (key, value))
        def __validateName(name: str, citem: dict) -> tuple:
        ...
        def __validatePath(paths: list, citem: dict) -> tuple:
            if len(paths) > 0:
                for path in paths:
        ...
        def __validateGroup(groups: list, citem: dict) -> tuple:
            return groups, Globals._cleanGroups(citem)
                   
                   
        def __validateRules(rules: dict, citem: dict) -> tuple:
            return rules, citem
    ...
    def validatePaths(self, path: str, citem: dict) -> tuple:
        def __convert(path: str) -> list: #convert videodb:// paths to dynamic xsp thru. predefined
            paths = [path]
        ...
        def __vfs(path: str, citem: dict, cnt: int = 3) -> bool:
            def __fileList(citem: dict, fileList: list = []) -> list:
                return Globals.properties.preemptActivity(LANGUAGE(32268), Builder(service=self).buildVideo, *(citem,True))

            with self.toggleSpinner(condition=Globals.properties.isRunning('Manager.toggleSpinner')==False):
                if Globals._isRadio({'path':[path]}): return True
                Globals.dialog.notificationDialog(LANGUAGE(32268))
                tmpcitem = citem.copy()
                tmpcitem.update({'name':FileAccess._getMD5(citem.get('path', [])),'path':[path]})
                ...
                if fileList:
                    while not self.monitor.abortRequested() and cnt > 0:
                        if __seek(random.choice(fileList)): return Globals.dialog.notificationDialog(LANGUAGE(32269))
                        else:
                            retval = Globals.dialog.yesnoDialog(LANGUAGE(30202),customlabel='Try Again (%s)'%(cnt))
                            if   retval == 1: return Globals.dialog.notificationDialog(LANGUAGE(32271))
                            elif retval == 2: cnt -=1
                            else: break
                self.log('validatePaths, __vfs: path = %s fileList = %s'%(path,len(fileList) if isinstance(fileList,list) else fileList))
                return not bool(Globals.dialog.notificationDialog(LANGUAGE(32270)))
                
        def __seek(item: dict, passed: bool = False, wait: int = 30) -> bool:
            player = PLAYER()
            if player.isPlaying(): return Globals.dialog.notificationDialog(LANGUAGE(32272))
            else:
        ...
        def __set(path: str, citem: dict) -> tuple:
        ...
        if path: 
            ...
            for p in paths:
                if __vfs(p, citem): return __set(p, citem)
        return None, citem
    ...
    def clrLibrary(self) -> bool:
        return Library().clrLibraryCache()


    def autoTune(self, start: int = 1):
        with self.toggleSpinner(condition=Globals.properties.isRunning('Manager.toggleSpinner')==False):
            try:
    ...
    def selectPredefined(self, start: int = 1):
        def __buildMenuItem(citem: dict) -> xbmcgui.ListItem:
            return Globals.listitems.buildMenuListItem(citem.get('name',''),citem.get('type',''),citem.get('logo',LOGO_COLOR),'|'.join(citem.get('path',[])),props={'citem':citem})
        try:
            with self.toggleSpinner(condition=Globals.properties.isRunning('Manager.toggleSpinner')==False):
            ...
            idx = Globals.dialog.selectDialog(types, header=ADDON_NAME, multi=False)
            if idx is None: return
            type = types[idx].getLabel()
    ...
    def _addChannels(self, start: int = 1, items: list = []) -> bool:
        if start > 0 and items:
            self.log(f'_addChannels, start = {start}, items = {len(items)}')
            self.log(f'_addChannels, items = {[{"name":i.get("name"),"type":i.get("type"),"path":i.get("path")} for i in items[:5]]}')
    ...
    def openEditor(self, path: str, media: str = 'video'):
        """Open the appropriate editor for the given path (XSP or Node)."""
        self.log('openEditor, path = %s'%(path))
        if '|' in path: 
            path = path.split('|')
            path = path[0]#prompt user to select:
        media = 'video' if 'video' in path else 'music'
        if   '.xsp' in path: Globals.builtin.executebuiltin('Addon.OpenSettings(script.globalsearch)') #todo open xsp editor
        elif '.xml' in path: return self.openNode(path,media)
       


    def previewChannel(self, citem: dict, retCntrl: Optional[int] = None) -> Optional[xbmcgui.ListItem]:
        def __buildItem(fitem: dict) -> xbmcgui.ListItem:
        ...
        def __fileList(citem: dict) -> tuple:
            fileList = []
            try:
                Globals.dialog.notificationDialog(LANGUAGE(32273).format(name=citem.get('name','Untitled')))
                tmpcitem = citem.copy()
                tmpcitem['id'] = Globals._getChannelID(citem.get('name',''), citem.get('path',[]), random.random())
                start_time = time.time()
                fileList   = Globals.properties.preemptActivity(LANGUAGE(32268), Builder(service=self).buildChannels, *([tmpcitem],True,False,False))
                end_time   = time.time()
    ...
    def getMontiorList(self) -> xbmcgui.ListItem:
        ...
        try:
            with self.toggleSpinner(condition=Globals.properties.isRunning('Manager.toggleSpinner')==False):
                ...
                itemSEL = Globals.dialog.selectDialog(itemLST,LANGUAGE(32078).format(name='Name'),useDetails=True,multi=False)
                if itemSEL is not None: return itemLST[itemSEL]
                else: raise Exception()
        except Exception: return xbmcgui.ListItem(LANGUAGE(32079))


    def switchLogo(self, channelData: dict, channelPOS: int):
        def __cleanLogo(chlogo: str) -> str:
        ...
        def __select() -> Optional[str]:
            def __buildItem(logo: str) -> xbmcgui.ListItem:
        ...
        def __browse() -> str:
            ...
            dest = os.path.join(LOGO_LOC,'%s%s'%(chname,retval[-4:])).replace('\\','/')
            if FileAccess.copy(__cleanLogo(retval), dest): 
                return dest
            return retval
    ...
    def getControlID(self, cntrl: Any) -> int:
    ...
    def isVisible(self, cntrl: Any) -> bool:
        state = False
        try: 
    ...
    def setVisibility(self, cntrl: Any, state: bool) -> bool:
        try: 
            if isinstance(cntrl, int):       cntrl = self.getControl(cntrl)
    ...
    def getLabels(self, cntrl: Any) -> tuple:
        try:
            if isinstance(cntrl, int): cntrl = self.getControl(cntrl)
    ...
    def setImages(self, cntrl: Any, image: str = 'NA.png'):
        try: 
            if isinstance(cntrl, int): cntrl = self.getControl(cntrl)
    ...
    def setEnableCondition(self, cntrl: Any, condition: str):
        try: 
            if isinstance(cntrl, int): cntrl = self.getControl(cntrl)
    ...
    def clearChannel(self, citem: dict, channelPOS: int, prompt: bool = True, open: bool = False):
        self.log('clearChannel, channelPOS = %s'%(citem.get('number', channelPOS + 1) - 1))
        with self.toggleSpinner(condition=Globals.properties.isRunning('Manager.toggleSpinner')==False):
            ...
            tmpItem['number'] = citem.get('number', channelPOS + 1) #preserve channel number
            self.newChannels[channelPOS] = tmpItem
            self.saveChannelItems(tmpItem, open)
            


    def moveChannel(self, citem: dict, channelPOS: int):
        """Move a channel to a new position in the channel list."""
        self.log('moveChannel, channelPOS = %s'%(channelPOS))
        if citem.get('number', 0) > CHANNEL_LIMIT: return Globals.dialog.notificationDialog(LANGUAGE(32064))
        retval = Globals.dialog.inputDialog(LANGUAGE(32137), key=xbmcgui.INPUT_NUMERIC, opt=citem.get('number', 0))
        if retval:
            retval = int(retval)
            if (retval > 0 and retval <= CHANNEL_LIMIT) and retval != channelPOS + 1:
                if Globals.dialog.yesnoDialog('%s %s %s from [B]%s[/B] to [B]%s[/B]?'%(LANGUAGE(32136),citem.get('name','Untitled'),LANGUAGE(32023),citem.get('number',0),retval)):
                    with self.toggleSpinner(condition=Globals.properties.isRunning('Manager.toggleSpinner')==False):
    ...
    def closeChannel(self, citem: dict, focus: int = 0, open: bool = False):
    ...
    def saveChannelItems(self, citem: dict = {}, open: bool = False) -> dict:
        ...
        if self.madeItemchange:
            ...
            with self.toggleSpinner(condition=Globals.properties.isRunning('Manager.toggleSpinner')==False):
                ...
                chnum = citem.get('number', 0)
                if chnum > 0:
                    self.newChannels[chnum - 1] = citem
        if self.launchManager:
            chnum = citem.get('number', 0)
            self.fillChanList(self.newChannels,True,max(0, chnum - 1),citem if open else None)
        return citem
    ...
    def saveChanges(self, start: int = 1, close: bool = True):
        ...
        def __validateChannels(newChannels: list) -> list:
            def __validate(citem: dict) -> Optional[dict]:
                ...
                with self.toggleSpinner(condition=Globals.properties.isRunning('Manager.toggleSpinner')==False):
                    ...
                    if self.server: #remote save
                        try:
                            ...
                            self.log('saveChanges, remote save failed: %s' % e, xbmc.LOGERROR)
                            Globals.dialog.notificationDialog(LANGUAGE(32000))  # "Something went wrong..."
                    else: #local save
