        ...
        if not cacheResponse:
            for id in list(dict.fromkeys(resources)):
                if MONITOR.waitForAbort(.001): 
                    ...
                    results = self.jsonRPC.walkListDirectory(os.path.join('special://home/addons/%s/resources'%id), exts=IMG_EXTS, depth=5, checksum=self.jsonRPC.getAddonDetails(id).get('version',ADDON_VERSION), expiration=datetime.timedelta(days=MAX_GUIDEDAYS))
