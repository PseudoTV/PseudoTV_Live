...
def checkPVR():
    ...
    check = [addon.getSetting('m3uRefreshMode')         == '1',
             ...
             addon.getSetting('m3uRefreshIntervalMins') == '30',
...
def configurePVR(override=False):
    ...
    if not override:
        if not yesnoDialog('%s ?'%(LANGUAGE(30074)%(getPluginMeta(PVR_CLIENT).get('name',''),ADDON_NAME,))): return
    try:
        addon = getPVR()
